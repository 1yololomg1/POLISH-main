...
router.post('/process', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        // Process the file
        const result = await processingService.processFile(buffer, fileName, processingOptions, userId);
        if (result.success) {
            res.json({
                success: true,
 ...
 */
router.get('/status/:jobId', auth_1.authenticateToken, async (req, res) => {
    try {
