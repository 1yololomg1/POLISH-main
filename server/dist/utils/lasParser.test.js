...
const lasParser_1 = require("./lasParser");
const MnemonicStandardizer_1 = require("./MnemonicStandardizer");
const globals_1 = require("@jest/globals");
(0, globals_1.describe)('LAS Parser - Enterprise Test Suite', () => {
    let parser;
    let standardizer;
    (0, globals_1.beforeEach)(() => {
        parser = new lasParser_1.LASParser();
        standardizer = new MnemonicStandardizer_1.MnemonicStandardizer();
    });
    (0, globals_1.describe)('Complex LAS 2.0 File with Multiple Challenges', () => {
...
#End of data section`;
        (0, globals_1.it)('should parse complex LAS 2.0 file with multiple challenges', async () => {
            ...
            (0, globals_1.expect)(result.parseTime).toBeLessThan(1000); // Should parse quickly
            const lasFile = result.data;
            // Verify header information
            (0, globals_1.expect)(lasFile.header.version).toBe('2.0');
            (0, globals_1.expect)(lasFile.header.well).toBe('North Sea Well-42');
            (0, globals_1.expect)(lasFile.header.field).toBe('Ekofisk Field');
            (0, globals_1.expect)(lasFile.header.startDepth).toBe(1670.0);
            (0, globals_1.expect)(lasFile.header.stopDepth).toBe(2450.0);
            (0, globals_1.expect)(lasFile.header.step).toBe(0.1524);
            // Verify curve information
            (0, globals_1.expect)(lasFile.curves.length).toBe(11); // Excluding depth
            (0, globals_1.expect)(lasFile.curves.find(c => c.mnemonic === 'GR')).toBeDefined();
            (0, globals_1.expect)(lasFile.curves.find(c => c.mnemonic === 'NPHI')).toBeDefined();
            (0, globals_1.expect)(lasFile.curves.find(c => c.mnemonic === 'RHOB')).toBeDefined();
            (0, globals_1.expect)(lasFile.curves.find(c => c.mnemonic === 'RT')).toBeDefined();
            // Verify data quality
            (0, globals_1.expect)(lasFile.data.length).toBeGreaterThan(30);
            (0, globals_1.expect)(lasFile.qualityScore).toBeGreaterThan(70);
            // Verify curve statistics
            const grCurve = lasFile.curves.find(c => c.mnemonic === 'GR');
            (0, globals_1.expect)(grCurve.statistics).toBeDefined();
            (0, globals_1.expect)(grCurve.statistics.min).toBeGreaterThan(20);
            (0, globals_1.expect)(grCurve.statistics.max).toBeLessThan(100);
            (0, globals_1.expect)(grCurve.statistics.nullCount).toBeGreaterThan(0);
            // Verify warnings for data quality issues
            (0, globals_1.expect)(result.warnings.some(w => w.includes('tool sticking'))).toBe(true);
            (0, globals_1.expect)(result.warnings.some(w => w.includes('gas effect'))).toBe(true);
            (0, globals_1.expect)(result.warnings.some(w => w.includes('missing data'))).toBe(true);
        });
        (0, globals_1.it)('should handle mnemonic standardization correctly', async () => {
            const buffer = Buffer.from(complexLAS2Content, 'utf8');
            const options = {
                autoStandardizeMnemonics: true,
                mnemonicStandard: 'api',
            ...
            const grCurve = lasFile.curves.find(c => c.mnemonic === 'GR');
            (0, globals_1.expect)(grCurve.standardMnemonic).toBeDefined();
        });
    });
    (0, globals_1.describe)('LAS 3.0 File with Unicode and Extended Features', () => {
...
1002.000,65.8,-0.30,2.85,55.2,30.1,20.1,6.5,-70,6.7,125,195`;
        (0, globals_1.it)('should parse LAS 3.0 file with comma delimiter', async () => {
            ...
            (0, globals_1.expect)(lasFile.header.version).toBe('3.0');
            (0, globals_1.expect)(lasFile.header.well).toBe('Deepwater Gulf Well-789');
            (0, globals_1.expect)(lasFile.header.field).toBe('Thunder Horse Field');
            // Verify data parsing with comma delimiter
            (0, globals_1.expect)(lasFile.data.length).toBeGreaterThan(20);
            (0, globals_1.expect)(lasFile.curves.length).toBe(11);
            // Verify high-temperature, high-pressure conditions
            (0, globals_1.expect)(lasFile.header.serviceCompany).toBe('Halliburton');
        });
    });
    (0, globals_1.describe)('Edge Cases and Error Handling', () => {
        (0, globals_1.it)('should handle empty file gracefully', async () => {
            const buffer = Buffer.from('', 'utf8');
            const result = await parser.parse(buffer, 'empty.las');
            (0, globals_1.expect)(result.success).toBe(false);
            (0, globals_1.expect)(result.error).toContain('empty');
        });
        (0, globals_1.it)('should handle file with only comments', async () => {
            const buffer = Buffer.from('# This is a comment\n# Another comment\n', 'utf8');
            const result = await parser.parse(buffer, 'comments_only.las');
            (0, globals_1.expect)(result.success).toBe(false);
            (0, globals_1.expect)(result.error).toContain('no valid content');
        });
        (0, globals_1.it)('should handle malformed section headers', async () => {
...
1000.200  52.1`;
            ...
            (0, globals_1.expect)(result.success).toBe(true); // Should still parse with warnings
            (0, globals_1.expect)(result.warnings.length).toBeGreaterThan(0);
        });
        (0, globals_1.it)('should handle large file size limits', async () => {
...
~ASCII Log Data Section`;
            ...
            (0, globals_1.expect)(result.success).toBe(false);
            (0, globals_1.expect)(result.error).toContain('exceeds maximum allowed size');
        });
        (0, globals_1.it)('should handle mixed data quality and null values', async () => {
...
1010.000  98.5  -0.15  3.05`;
            ...
            (0, globals_1.expect)(lasFile.qualityScore).toBeLessThan(100);
        });
    });
    (0, globals_1.describe)('Performance and Memory Management', () => {
...
~ASCII Log Data Section`;
            ...
            (0, globals_1.expect)(result.success).toBe(true);
            (0, globals_1.expect)(parseTime).toBeLessThan(5000); // Should parse in under 5 seconds
            (0, globals_1.expect)(result.data.data.length).toBe(10000);
        });
    });
    (0, globals_1.describe)('Industry Standards Compliance', () => {
        (0, globals_1.it)('should validate physical ranges correctly', async () => {
...
1010.000  95.2  -0.18  3.12  41.5`;
            ...
            (0, globals_1.expect)(result.success).toBe(true);
            (0, globals_1.expect)(result.warnings.some(w => w.includes('GR'))).toBe(true);
            (0, globals_1.expect)(result.warnings.some(w => w.includes('NPHI'))).toBe(true);
            (0, globals_1.expect)(result.warnings.some(w => w.includes('RHOB'))).toBe(true);
            (0, globals_1.expect)(result.warnings.some(w => w.includes('RT'))).toBe(true);
        });
    });
});
//# sourceMappingURL=lasParser.test.js.map